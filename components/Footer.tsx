...
export default function Footer({ settings = defaults }: { settings?: typeof defaults }) {
    ...
    <footer id="site-footer" className="relative min-h-screen w-full bg-bg0 border-t border-line flex flex-col overflow-hidden">
      ...
      <div className="relative flex-1 flex flex-col justify-center max-w-[1400px] w-full mx-auto px-6 md:px-16 pt-[130px] pb-[30px]">
        <h2 className="m-0 text-fg tracking-[-0.035em] leading-[0.9]">
          <span className="block font-extralight text-[clamp(40px,5.4vw,76px)]">{settings.footerCtaLine1}</span>
          <span className="block font-extrabold text-[clamp(60px,9vw,132px)]">{settings.footerCtaLine2}</span>
        </h2>
        <div className="flex flex-wrap items-center gap-[18px] mt-11">
          {settings.whatsappUrl && <a href={settings.whatsappUrl} className="inline-flex items-center gap-2.5 px-[38px] py-5 bg-brand text-white font-semibold text-[17px] rounded-full no-underline transition-transform hover:-translate-y-[3px]">Escribinos por WhatsApp →</a>}
      ...
      <div className="relative max-w-[1400px] w-full mx-auto px-6 md:px-16 py-9 grid grid-cols-1 md:grid-cols-[1.5fr_1fr_1fr] gap-12 items-start border-t border-line3">
        <div>
          <p className="m-0 text-muted text-sm font-light leading-relaxed max-w-[260px]">
            {addr.map((l, i) => <span key={i}>{l}{i < addr.length - 1 && <br />}</span>)}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3.5">
          {settings.whatsappUrl && <a href={settings.whatsappUrl} className="text-brand text-sm no-underline">{settings.whatsappLabel}</a>}
          {settings.instagram && <a href={settings.instagram} className="text-brand text-sm no-underline">Instagram ↗</a>}
          {settings.linkedin && <a href={settings.linkedin} className="text-brand text-sm no-underline">LinkedIn ↗</a>}
          {settings.youtube && <a href={settings.youtube} className="text-brand text-sm no-underline">YouTube ↗</a>}
        </div>
      </div>
      ...
      <div className="relative flex justify-between items-center max-w-[1400px] w-full mx-auto px-6 md:px-16 py-[18px] border-t border-line3">
        <span className="text-muted text-xs">{settings.copyright}</span>
