...
const STEPS = [
  { badge: "Reducir · Rediseñar", title: "Ecodiseño de envases y productos", text: "Repensamos el producto desde su origen: menos material, más vida útil y pensado para volver a empezar.", slot: "proc-1" },
  { badge: "I+D+i", title: "Investigación y desarrollo", text: "De la idea a una solución viable: investigamos materiales, validamos hipótesis y desarrollamos el diseño.", slot: "proc-2" },
  { badge: "Prototipo", title: "Matricería y prototipado", text: "Materializamos el diseño con precisión: matrices, moldes y prototipos funcionales listos para producir.", slot: "proc-3" },
  { badge: "100% reciclado", title: "Producción con material reciclado", text: "Producimos con materia prima reciclada, trazable y local. Tintas al agua y monomaterial para reciclar otra vez.", slot: "proc-4" },
  { badge: "Circular", title: "Consultoría en economía circular", text: "Acompañamos a marcas y eventos a cerrar el ciclo: de residuos a recursos, con impacto medible.", slot: "proc-5" },
];
...
const ImageBox = ({ slot, title }: { slot: string; title: string }) => (
  ...
  return (
    <section id="metodologia" className="relative w-full bg-bg2 pt-[130px] pb-[150px] px-6 md:px-16 overflow-hidden">
      ...
      <div className="max-w-[1080px] mx-auto mb-20 text-center relative">
        <span className="block text-xs tracking-[0.22em] uppercase text-brand font-bold">Metodología</span>
        <h2 className="mt-4 text-fg tracking-[-0.03em] text-[clamp(40px,7vw,60px)] leading-none"><span className="font-extralight">Cómo </span><span className="font-extrabold">trabajamos.</span></h2>
        <p className="mt-5 mx-auto max-w-[560px] text-muted text-lg font-light leading-[1.5]">Un proceso circular: cada paso conecta con el siguiente para convertir el residuo en un nuevo recurso.</p>
      </div>
      ...
      <div ref={trackRef} className="relative max-w-[1080px] mx-auto">
        ...
        {STEPS.map((s, i) => {
          ...
          const Img = <div className={textLeft ? "md:pl-12" : "md:pr-12"}><ImageBox slot={s.slot} title={s.title} /></div>;
          return (
