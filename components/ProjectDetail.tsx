...
type Project = {
  slug: string; tag?: string; title: string; subtitle?: string;
  heroImage?: Img; featuredImage?: Img; gallery?: Img[];
  intro?: { lead?: string; body?: string };
  meta?: { cliente?: string; anio?: string; rol?: string; pilares?: string };
  desafio?: string; solucion?: string;
  proceso?: Step[]; impacto?: { stat?: string; text?: string }[]; ficha?: { k?: string; v?: string }[];
};
type Adj = { slug: string; title: string };

export default function ProjectDetail({ project, prev, next }: { project: Project; prev: Adj; next: Adj }) {
    ...
    <main>
      <section className="relative h-screen overflow-hidden bg-[#0d0d0c]">
        <div ref={bgRef} className="absolute inset-[-10%_-2%] will-change-transform scale-[1.12]">
          <SmartImage img={project.heroImage} alt={project.title} />
        </div>
      ...
      <Reveal as="section" className="w-full pt-[30px] pb-2.5">
        <div className="relative w-[calc(100%-48px)] max-w-[1500px] mx-auto aspect-[16/9] rounded-[24px] overflow-hidden shadow-[0_40px_90px_rgba(0,0,0,0.22)] group">
          <SmartImage img={project.featuredImage} alt={`${project.title} — destacada`} className="transition-transform duration-700 group-hover:scale-[1.06]" />
        </div>
      </Reveal>
        ...
        <div ref={galRef} className="hscroll mt-7 flex gap-4 overflow-x-auto snap-x snap-mandatory px-6 md:px-16 pb-3.5 cursor-grab active:cursor-grabbing">
          {[0, 1, 2].map((i) => (
            <div key={i} className="relative flex-none w-[min(80vw,520px)] aspect-[4/5] rounded-[18px] overflow-hidden snap-center shadow-[0_24px_56px_rgba(0,0,0,0.18)] group">
              <SmartImage img={gallery[i]} alt={`${project.title} ${i + 1}`} className="transition-transform duration-700 group-hover:scale-[1.06]" />
            </div>
          ))}
        </div>
      </Reveal>
      ...
      <section className="max-w-[1160px] mx-auto px-6 md:px-16 py-[70px]">
        ...
        <div className="flex flex-wrap gap-7 mt-9">
          {(project.proceso || []).map((s, i) => (
            <Reveal key={i} className="flex-[1_1_210px] border-t-2 border-brand pt-[18px]">
        ...
        <Reveal className="flex flex-wrap gap-12 items-start">
          ...
          <div className="flex-[2_1_420px]">
            {(project.ficha || []).map((r, i) => (
              <div key={i} className="flex justify-between gap-5 py-[18px] border-t border-line">
